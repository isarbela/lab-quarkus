...
public class RedisElectionRepository implements ElectionRepository {
    ...
    public Election sync(Election election) {
        var map = commands.zrangebyscoreWithScores("election:" + election.id(),
                ScoreRange.from(Integer.MIN_VALUE, Integer.MAX_VALUE)).stream()
                .map(stringScoredValue -> {
                    Candidate candidate = election
                            .votes()
                            .keySet()
                            .stream()
                            .filter(c -> c.id().equals(stringScoredValue.value))
                            .findFirst()
                            .orElseThrow();

                    return Map.entry(candidate, (int) stringScoredValue.score());
