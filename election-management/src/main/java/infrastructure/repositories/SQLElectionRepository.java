    ...
    public void submit(Election election) {
        ...
        election.votes()
                ...
                .map(entry -> ElectionCandidate
                        .fromDomain(election, entry.getKey(), entry.getValue())).forEach(entityManager::merge);
    }

    public List<Election> findAll() {
        Stream<Object[]> stream = entityManager.createNativeQuery("SELECT e.id " +
                        "AS election_id, c.id " +
                        "AS candidate_id, c.photo, c.given_name, c.family_name, c.email, c.phone, c.job_title, ec.votes " +
                        "FROM elections AS e " +
                        "INNER JOIN election_candidate " +
                        "AS ec ON ec.election_id = e.id " +
